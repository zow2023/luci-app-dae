...
return view.extend({
    load: function () {
        return utils.ensureAllConfigs().then(function () {
            return fs.read('/etc/dae/config.d/node.dae');
        });
    },
    ...
    render: function (configData) {
        ...
        m = new form.Map('dae', _('Node Settings'), _('Configure nodes and groups for DAE.'));
        ...
        o = s.option(form.TextValue, 'nodeconf', _('Node Configuration'), _('Configure nodes and groups for DAE. Click "Save" to apply changes to /etc/dae/config.d/node.dae.'));
        o.rows = 25;
        o.rmempty = true;
        o.monospace = true;
        ...
        o.write = function (section_id, formvalue) {
            return utils.ensureAllConfigs().then(function () {
                return fs.write('/etc/dae/config.d/node.dae', formvalue.replace(/\r\n/g, '\n'));
            });
        };
